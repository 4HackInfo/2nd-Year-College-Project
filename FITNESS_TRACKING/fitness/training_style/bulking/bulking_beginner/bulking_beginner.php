...
let data = JSON.parse(localStorage.getItem("bulking_beginner_progress")) || defaultData;

function saveData() {
  localStorage.setItem("bulking_beginner_progress", JSON.stringify(data));
}
...
function createDayCard(dayNum) {
  ...
  if (!isUnlockedDay && !isVisited) {
    ...
    img.src = (dayNum % 2 === 0) ? "rest.png" : "workout3.png";
  }
  ...
  card.onclick = () => {
    ...
    window.location.href = `day${dayNum}/day${dayNum}_bulking_beginner.php`;
  };
...
function resetProgress() {
  localStorage.removeItem("bulking_beginner_progress");
  location.reload();
}
</script>
