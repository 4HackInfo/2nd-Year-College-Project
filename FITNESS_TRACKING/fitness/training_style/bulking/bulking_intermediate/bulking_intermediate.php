...
let data = JSON.parse(localStorage.getItem("bulking_intermediate_progress")) || defaultData;

function saveData() {
  localStorage.setItem("bulking_intermediate_progress", JSON.stringify(data));
}
...
function createDayCard(dayNum) {
  ...
  if (!isUnlockedDay && !isVisited) {
    ...
    img.src = (dayNum % 2 === 0) ? "rest.png" : "workout4.png";
  }
  ...
  card.onclick = () => {
    ...
    window.location.href = `day${dayNum}/day${dayNum}_bulking_intermediate.php`;
  };
...
function resetProgress() {
  localStorage.removeItem("bulking_intermediate_progress");
  location.reload();
}
</script>
