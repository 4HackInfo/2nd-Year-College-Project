...
let data = JSON.parse(localStorage.getItem("cardio_advanced_progress")) || defaultData;

function saveData() {
  localStorage.setItem("cardio_advanced_progress", JSON.stringify(data));
}
...
function createDayCard(dayNum) {
  ...
  if (!isUnlockedDay && !isVisited) {
    ...
    img.src = (dayNum % 2 === 0) ? "rest.png" : "workout8.png";
  }
  ...
  card.onclick = () => {
    ...
    window.location.href = `day${dayNum}/day${dayNum}_cardio_advanced.php`;
};
...
localStorage.removeItem("cardio_advanced_progress");
location.reload();
}
</script>
