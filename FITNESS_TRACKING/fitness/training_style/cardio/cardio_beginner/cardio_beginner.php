...
let data = JSON.parse(localStorage.getItem("cardio_beginner_progress")) || defaultData;

function saveData() {
  localStorage.setItem("cardio_beginner_progress", JSON.stringify(data));
}
...
function createDayCard(dayNum) {
  ...
  if (!isUnlockedDay && !isVisited) {
    ...
    img.src = (dayNum % 2 === 0) ? "rest.png" : "workout6.png";
  }
  ...
  card.onclick = () => {
    ...
    window.location.href = `day${dayNum}/day${dayNum}_cardio_beginner.php`;
};
...
localStorage.removeItem("cardio_beginner_progress");
location.reload();
}
</script>
