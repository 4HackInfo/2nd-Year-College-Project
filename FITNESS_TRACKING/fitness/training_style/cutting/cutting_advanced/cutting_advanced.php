...
let data = JSON.parse(localStorage.getItem("cutting_advanced_progress")) || defaultData;

function saveData() {
  localStorage.setItem("cutting_advanced_progress", JSON.stringify(data));
}
...
function createDayCard(dayNum) {
  ...
  if (!isUnlockedDay && !isVisited) {
    ...
    img.src = (dayNum % 2 === 0) ? "rest.png" : "workout1.png";
  }
  ...
  card.onclick = () => {
    ...
    window.location.href = `day${dayNum}/day${dayNum}_view_advanced.php`;
};
...
localStorage.removeItem("cutting_advanced_progress");
location.reload();
}
</script>
