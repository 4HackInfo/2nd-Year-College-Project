...
let data = JSON.parse(localStorage.getItem("cutting_beginner_progress")) || defaultData;

function saveData() {
  localStorage.setItem("cutting_beginner_progress", JSON.stringify(data));
}
...
function createDayCard(dayNum) {
  ...
  if (!isUnlockedDay && !isVisited) {
    ...
    img.src = (dayNum % 2 === 0) ? "rest.png" : "workout.png";
  }
  ...
  card.onclick = () => {
    ...
    window.location.href = `day${dayNum}/day${dayNum}_view.php`;
  };
...
function resetProgress() {
  localStorage.removeItem("cutting_beginner_progress");
  location.reload();
}
</script>
