...
let data = JSON.parse(localStorage.getItem("cutting_intermediate_progress")) || defaultData;

function saveData() {
  localStorage.setItem("cutting_intermediate_progress", JSON.stringify(data));
}
...
function createDayCard(dayNum) {
  ...
  if (!isUnlockedDay && !isVisited) {
    ...
    img.src = (dayNum % 2 === 0) ? "rest.png" : "workout2.png";
  }
  ...
  card.onclick = () => {
    ...
    window.location.href = `day${dayNum}/day${dayNum}_view_intermediate.php`;
  };
...
function resetProgress() {
  localStorage.removeItem("cutting_intermediate_progress");
